...
static void show_textbox( char *title,  char *text, int r, int c);
static void show_helptext( char *title,  char *text);
static void show_help(struct menu *menu);
static void show_file( char *filename,  char *title, int r, int c);
...
static int cprint_name( char *fmt, ...)
{
	va_list ap;
	int res;
...
static int cprint_tag( char *fmt, ...)
{
	va_list ap;
	int res;
...
static void build_conf(struct menu *menu)
{
	...
	if (!sym) {
		if (prop && menu != current_menu) {
			 char *prompt = menu_get_prompt(menu);
			switch (prop->type) {
			case P_MENU:
				child_count++;
				cmake();
